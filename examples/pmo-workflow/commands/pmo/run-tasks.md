...
1. `Source URL` プロパティが存在する場合:
   - `Source URL` から Confluence ページIDを抽出する
     - URL 形式例: `https://YOUR_DOMAIN.atlassian.net/wiki/spaces/KEY/pages/123456789/...`
     - `/pages/` の直後の数字部分（例: `123456789`）をページIDとして使用
...
- `mcp__claude_ai_Atlassian__updateJiraIssue` — Jira チケット内容更新
- `mcp__claude_ai_Atlassian__transitionJiraIssue` — Jira チケットステータス変更
